...
namespace vgui
{
	...
	//-----------------------------------------------------------------------------
	class LabelEx : public Panel
	{
		DECLARE_CLASS_SIMPLE(LabelEx, Panel);
	...
	public:
		...
		// alignment code, used to determine how the images are layed out within the LabelExEx
		enum Alignment
		{
			a_northwest = 0,
			a_north,
		...
		// LabelExExs can be associated with controls, and alter behaviour based on the associates behaviour
		...
		virtual TextImageEx* GetTextImage();
		...
		// for LabelExExs "text" is an option that returns the default text image text
	...
	private:
		...
		TextImageEx* _textImage; // this is the textImage, if the full text will not
