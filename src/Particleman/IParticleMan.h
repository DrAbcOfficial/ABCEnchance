...
public:
	...
	virtual void SetVariables(float flGravity, vec3_t vViewAngles) = 0;
	virtual void ResetParticles(void) = 0;
	virtual void ApplyForce(vec3_t vOrigin, vec3_t vDirection, float flRadius, float flStrength, float flDuration) = 0;
	...
	virtual CBaseParticle* CreateParticle(vec3_t org, vec3_t normal, model_s* sprite, float size, float brightness, const char* classname) = 0;
	...
	virtual void CoreInitializeSprite(CCoreTriangleEffect* pParticle, vec3_t org, vec3_t normal, model_s* sprite, float size, float brightness) = 0; //Only use this for TrianglePlanes
	...
	virtual void CoreTouch(CCoreTriangleEffect* pParticle, vec3_t pos, vec3_t normal, int index) = 0;
...
public:
	...
	virtual void InitializeSprite(vec3_t org, vec3_t normal, model_s* sprite, float size, float brightness)
	{
