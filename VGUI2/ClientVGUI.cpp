...
void ClientVGUI_InstallHook(void)
{
	...
	if(g_dwClientBase)
		ClientVGUICreateInterface = (CreateInterfaceFn)Sys_GetFactory((HINTERFACEMODULE)g_dwClientBase);
...
void (*__fastcall R_vgui_TextImage_paint)	(vgui1_TextImage* pthis, int, void* panel);
...
void __fastcall vgui_TextImage_paint(vgui1_TextImage *pthis, int, void *panel)
{
	...
	return R_vgui_TextImage_paint(pthis, 0, panel);
}
...
void VGUI1_InstallHook(void)
{
	...
	if (g_hVGui1)
	{
		...
		R_vgui_TextImage_paint = (decltype(R_vgui_TextImage_paint))vftable_TextImage[22];

		g_pMetaHookAPI->InlineHook((void*)R_vgui_TextImage_paint, vgui_TextImage_paint, (void**)&R_vgui_TextImage_paint);
	}
}
