...
//=============================================================================
#include "metahook.h"
#include "GameUI/OptionsSubButtonSettings.h"
...
void COptionsSubButtonSettings::UpdateButtonList()
{
	...
	gEngfuncs.pfnClientCmd("touch_list\n");
}
...
void COptionsSubButtonSettings::UpdateProfileList()
{
	...
	const char* pszFilename = vgui::filesystem()->FindFirst("touch_presets/*.cfg", &findHandle);
	while (pszFilename)
	{
		m_vecProfiles.push_back(pszFilename);
		m_pButtonProfileList->AddItem(pszFilename, NULL);
		pszFilename = vgui::filesystem()->FindNext(findHandle);
	}
	vgui::filesystem()->FindClose(findHandle);

	findHandle = NULL;
	pszFilename = vgui::filesystem()->FindFirst("touch_profiles/*.cfg", &findHandle);
	while (pszFilename)
	{
		m_vecProfiles.push_back(pszFilename);
		m_pButtonProfileList->AddItem(pszFilename, NULL);
...
COptionsSubButtonSettings::COptionsSubButtonSettings(vgui::Panel *parent) : PropertyPage(parent, NULL)
{
	m_pButtonRSlider = new vgui::Slider(this, "ColorR Slider");
	m_pButtonRSlider->SetRange(0, 255);

	m_pButtonGSlider = new vgui::Slider(this, "ColorG Slider");
	m_pButtonGSlider->SetRange(0, 255);

	m_pButtonBSlider = new vgui::Slider(this, "ColorB Slider");
	m_pButtonBSlider->SetRange(0, 255);

	m_pButtonASlider = new vgui::Slider(this, "ColorAlpha Slider");
	m_pButtonASlider->SetRange(0, 255);

	m_pCommend = new vgui::TextEntry(this, "Button Commend");
	m_pTexture = new vgui::TextEntry(this, "Button Texture");
	m_pButtonName = new vgui::TextEntry(this, "Button Name");

	m_pButtonHide = new vgui::CheckButton(this, "Button_Hide", "#Button_Hide");
	m_pButtonSp = new vgui::CheckButton(this, "Button_Sp", "#Button_Sp");
	m_pButtonMp = new vgui::CheckButton(this, "Button_Mp", "#Button_Mp");
	m_pButtonAdditive = new vgui::CheckButton(this, "Button_Additive", "#Button_Additive");
	m_pButtonLock = new vgui::CheckButton(this, "Button_Lock", "#Button_Lock");
	m_pButtonPrecision = new vgui::CheckButton(this, "Button_Precision", "#Button_Precision");

	m_pButtonSave = new vgui::Button(this, "Save", "#Button_Save");
	m_pButtonSave->SetCommand("ButtonSave");

	m_pButtonDelete = new vgui::Button(this, "Delete", "#Button_Delete");
	m_pButtonDelete->SetCommand("ButtonDelete");

	m_pButtonEditor = new vgui::Button(this, "Editor", "#Button_Editor");
	m_pButtonEditor->SetCommand("ButtonEditor");

	m_pButtonReset = new vgui::Button(this, "Reset", "#Button_Reset");
	m_pButtonReset->SetCommand("ButtonReset");

	m_pTextureList = new vgui::ComboBox(this, "TextureListComboBox", 6, false);
	m_pButtonList = new vgui::ComboBox(this,"ButtonListComboBox", 6, false);
	m_pButtonProfileList = new vgui::ComboBox(this, "ButtonProfileListComboBox", 6, false);

	m_pProfileActivate = new vgui::Button(this, "Profile Activate", "#Profile_Activate");
	m_pProfileActivate->SetCommand("ProfileActivate");

	m_pProfileDelete = new vgui::Button(this, "Profile Delete", "#Profile_Delete");
...
{
	char c;
	int scripting = gEngfuncs.pfnGetCvarFloat("cmd_scripting");
...
void COptionsSubButtonSettings::OnCommand(const char* command)
{
	if (!stricmp(command, "ButtonSave"))
	{
		...
		{
			auto msgbox = new vgui::MessageBox("提示", "请完善按钮信息（名称、贴图、指令）", this);
...
#define BUF_LENGTH 256
	...
	{
		gEngfuncs.pfnClientCmd("touch_enable \"1\";touch_in_menu \"1\";touch_enableedit");
	}
	else if (!stricmp(command, "ButtonReset"))
	{
		gEngfuncs.pfnClientCmd("touch_removeall\n");
		gEngfuncs.pfnClientCmd("touch_loaddefaults\n");
		UpdateButtonList();
	}
	...
	else if (!m_vecProfiles.empty())
	{
		if (!stricmp(command, "ProfileActivate"))
		{
			...
			auto msgbox = new vgui::MessageBox("提示", szTips, this);
			msgbox->SetOKButtonText("#CSO_OKl_Btn");
...
void COptionsSubButtonSettings::OnTextChanged(vgui::Panel* panel, const char* pszText)
{
	if (panel == m_pCommend)
		return;
