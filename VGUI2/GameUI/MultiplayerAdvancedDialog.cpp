...
void CMultiplayerAdvancedDialog::OnKeyCodeTyped(vgui::KeyCode code)
{
...
void CMultiplayerAdvancedDialog::GatherCurrentValues(void)
{
	...
	vgui::CheckButton *pBox;
	vgui::TextEntry *pEdit;
	...
	while (pList)
	{
		...
		switch (pObj->type)
		{
			case O_BOOL:
			{
				pBox = (vgui::CheckButton *)pList->pControl;
			...
			case O_NUMBER:
			{
				pEdit = (vgui::TextEntry *)pList->pControl;
			...
			case O_LIST:
			{
				pCombo = (vgui::ComboBox *)pList->pControl;
...
void CMultiplayerAdvancedDialog::CreateControls(void)
{
	...
	while (pObj)
	{
		...
		switch (pCtrl->type)
		{
			case O_BOOL:
			{
				pBox = new vgui::CheckButton(pCtrl, "DescCheckButton", pObj->prompt);
			...
			case O_NUMBER:
			{
				pEdit = new vgui::TextEntry(pCtrl, "DescTextEntry");
			...
			case O_LIST:
			{
				pCombo = new vgui::ComboBox(pCtrl, "DescComboBox", 5, false);
...
void CMultiplayerAdvancedDialog::SaveValues(void)
{
	...
	if (m_pDescription)
	{
		...
		FileHandle_t fp = vgui::filesystem()->Open("user.scr", "wb");

		if (fp)
		{
			m_pDescription->WriteToScriptFile(fp);
			vgui::filesystem()->Close(fp);
		}
	}
}
...
void CInfoDescription::WriteScriptHeader(void *fp)
{
	...
	vgui::filesystem()->FPrintf(fp, (char *)getHint());
	vgui::filesystem()->FPrintf(fp, "// Half-Life User Info Configuration Layout Script (stores last settings chosen, too)\r\n");
	vgui::filesystem()->FPrintf(fp, "// File generated:  %.19s %s\r\n", asctime(tblock), am_pm);
	vgui::filesystem()->FPrintf(fp, "//\r\n//\r\n// Cvar\t-\tSetting\r\n\r\n");
	vgui::filesystem()->FPrintf(fp, "VERSION %.1f\r\n\r\n", SCRIPT_VERSION);
	vgui::filesystem()->FPrintf(fp, "DESCRIPTION INFO_OPTIONS\r\n{\r\n");
}
...
void CInfoDescription::WriteFileHeader(void *fp)
{
	...
	vgui::filesystem()->FPrintf(fp, "// Half-Life User Info Configuration Settings\r\n");
	vgui::filesystem()->FPrintf(fp, "// DO NOT EDIT, GENERATED BY HALF-LIFE\r\n");
	vgui::filesystem()->FPrintf(fp, "// File generated:  %.19s %s\r\n", asctime(tblock), am_pm);
	vgui::filesystem()->FPrintf(fp, "//\r\n//\r\n// Cvar\t-\tSetting\r\n\r\n");
}

CInfoDescription::CInfoDescription(vgui::PanelListPanel*panel) : CDescription(panel)
{
