...
CLoadingDialog::CLoadingDialog(vgui::Panel* parent) : Frame(parent, "LoadingDialog")
{
	...
	m_pProgress2 = new vgui::ProgressBar(this, "Progress2");
	m_pInfoLabel = new vgui::Label(this, "InfoLabel", "");
	m_pCancelButton = new vgui::Button(this, "CancelButton", "#GameUI_Cancel");
	m_pTimeRemainingLabel = new vgui::Label(this, "TimeRemainingLabel", "");
	...
	m_pInfoLabel->SetTextColorState(vgui::Label::CS_DULL);
...
void CLoadingDialog::DisplayGenericError(const char* failureReason, const char* extendedReason)
{
	...
	if (extendedReason && strlen(extendedReason) > 0)
	{
		...
		if (extendedReason[0] == '#')
			wcsncpy(compositeReason, vgui::localize()->Find(extendedReason), sizeof(compositeReason) / sizeof(wchar_t));
		else
			vgui::localize()->ConvertANSIToUnicode(extendedReason, compositeReason, sizeof(compositeReason));

		if (failureReason[0] == '#')
			wcsncpy(formatStr, vgui::localize()->Find(failureReason), sizeof(formatStr) / sizeof(wchar_t));
		else
...
void CLoadingDialog::OnThink(void)
{
	...
	if (m_bShowingSecondaryProgress)
	{
		...
		else if (vgui::ProgressBar::ConstructTimeRemainingString(unicode, sizeof(unicode), m_flSecondaryProgressStartTime, (float)vgui::system()->GetFrameTime(), m_flSecondaryProgress, m_flLastSecondaryProgressUpdateTime, true))
...
void CLoadingDialog::PerformLayout(void)
{
	...
	else
	{
		...
		vgui::surface()->GetWorkspaceBounds(x, y, screenWide, screenTall);
...
void CLoadingDialog::SetSecondaryProgress(float progress)
{
	...
	if (!m_bShowingSecondaryProgress)
	{
		...
		m_flSecondaryProgressStartTime = (float)vgui::system()->GetFrameTime();
		m_pCancelButton->SetContentAlignment(vgui::Label::a_center);
	}
	...
	if (progress > m_flSecondaryProgress)
	{
		...
		m_flLastSecondaryProgressUpdateTime = (float)vgui::system()->GetFrameTime();
	}
...
void CLoadingDialog::OnClose(void)
{
	if (strlen(m_szBackgroundImage) > 0)
	{
		...
		m_pLoadingBackground->SetParent((vgui::VPANEL)NULL);
		m_pLoadingBackground->SetVisible(false);
	}
...
void CLoadingDialog::OnKeyCodePressed(vgui::KeyCode code)
{
