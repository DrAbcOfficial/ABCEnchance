...
class CrosshairImagePanel : public vgui::ImagePanel
{
...
void CrosshairImagePanel::Paint(void)
{
	...
	*/
	...
	if (bDrawCircle)
	{
		...
		{
			for (int i = 0; i < count; i++)
				gEngfuncs.pfnFillRGBA(x + (wide / 2) + radius * cos(2 * M_PI / count * i), y + (tall / 2) + radius * sin(2 * M_PI / count * i), 1, 1, m_R, m_G, m_B, 255);
		}
		else
		{
			for (int i = 0; i < count; i++)
				gEngfuncs.pfnFillRGBABlend(x + (wide / 2) + radius * cos(2 * M_PI / count * i), y + (tall / 2) + radius * sin(2 * M_PI / count * i), 1, 1, m_R, m_G, m_B, 255);
		}
	}
	...
	if (bDrawPoint)
	{
		if (additive)
		{
			gEngfuncs.pfnFillRGBA(x + (wide / 2) - 1, y + (tall / 2) - 1, 3, 3, m_R, m_G, m_B, 255);
		}
		else
		{
			gEngfuncs.pfnFillRGBABlend(x + (wide / 2) - 1, y + (tall / 2) - 1, 3, 3, m_R, m_G, m_B, 255);
		}
	}
	...
	if (bDrawCross)
	{
		if (additive)
		{
			gEngfuncs.pfnFillRGBA(x + (wide / 2 - m_barGap - m_barSize) + 1, y + tall / 2, m_barSize, 1, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBA(x + wide / 2 + m_barGap, y + tall / 2, m_barSize, 1, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBA(x + wide / 2, y + tall / 2 - m_barGap - m_barSize + 1, 1, m_barSize, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBA(x + wide / 2, y + tall / 2 + m_barGap, 1, m_barSize, m_R, m_G, m_B, 255);
		}
		else
		{
			gEngfuncs.pfnFillRGBABlend(x + (wide / 2 - m_barGap - m_barSize) + 1, y + tall / 2, m_barSize, 1, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBABlend(x + wide / 2 + m_barGap, y + tall / 2, m_barSize, 1, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBABlend(x + wide / 2, y + tall / 2 - m_barGap - m_barSize + 1, 1, m_barSize, m_R, m_G, m_B, 255);
			gEngfuncs.pfnFillRGBABlend(x + wide / 2, y + tall / 2 + m_barGap, 1, m_barSize, m_R, m_G, m_B, 255);
		}
	}
...
COptionsSubMultiplayer::COptionsSubMultiplayer(vgui::Panel *parent) : vgui::PropertyPage(parent, "OptionsSubMultiplayer")
{
	...
	vgui::Button *ok = new vgui::Button(this, "OK", "#GameUI_OK");
	ok->SetCommand("Ok");

	vgui::Button *apply = new vgui::Button(this, "Apply", "#GameUI_Apply");
	apply->SetCommand("Apply");

	vgui::Button *advanced = new vgui::Button(this, "Advanced", "#GameUI_AdvancedEllipsis");
	...
	m_pLogoImage = new vgui::CBitmapImagePanel(this, "LogoImage");
...
void COptionsSubMultiplayer::InitLogoList(CLabeledCommandComboBox *cb)
{
	...
	vgui::filesystem()->RemoveFile("logos/remapped.bmp", NULL);

	const char *logofile = gEngfuncs.pfnGetCvarString("cl_logofile");
	sprintf(directory, "logos/*.bmp");
	const char *fn = vgui::filesystem()->FindFirst(directory, &fh);
...
{
	char const *currentcolor = gEngfuncs.pfnGetCvarString("cl_logocolor");
...
void COptionsSubMultiplayer::InitCrosshairColorEntries(void)
{
	...
	int selected = 0;
	int cr = 0, cg = 0, cb = 0;
	const char *color = gEngfuncs.pfnGetCvarString("cl_crosshair_color");
...
void COptionsSubMultiplayer::InitCrosshairSizeList(CLabeledCommandComboBox *cb)
{
	...
	auto value = gEngfuncs.pfnGetCvarString("cl_crosshair_size");
...
void COptionsSubMultiplayer::InitCrosshairTypeList(CLabeledCommandComboBox *cb)
{
	...
	auto value = gEngfuncs.pfnGetCvarString("cl_crosshair_type");
...
void COptionsSubMultiplayer::RemapLogoPalette(char *filename, int r, int g, int b)
{
	...
	vgui::filesystem()->RemoveFile(outfile, NULL);

	vgui::filesystem()->CreateDirHierarchy("logos", NULL);
	auto file = vgui::filesystem()->Open(outfile, "wb");

	if (file != FILESYSTEM_INVALID_HANDLE)
	{
		vgui::filesystem()->Write(bmpFile->GetBitmap(), bmpFile->GetBitmapHdr()->fileSize, file);
...
/*
	...
	if (colorname && colorname[0])
	{
		...
		FileHandle_t file = vgui::filesystem()->Open(infile, "rb");
		...
		if (file != FILESYSTEM_INVALID_HANDLE)
		{
			...
			dwFileSize = vgui::filesystem()->Size(file);
			vgui::filesystem()->Read(&bmfHeader, sizeof(bmfHeader), file);
			...
			if (bmfHeader.bfType == DIB_HEADER_MARKER)
			{
				...
				vgui::filesystem()->Read(pDIB, dwBitsSize, file);
