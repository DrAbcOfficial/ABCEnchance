...
public:
    ...
    {
        int length = 0;
        FileHandle_t file = vgui::filesystem()->Open(filename, "rb");

        // cannot load
        if( file == FILESYSTEM_INVALID_HANDLE )
            return NULL;

        length = vgui::filesystem()->Size(file);
        ...
        vgui::filesystem()->Read(buffer.data(), length, file);
        vgui::filesystem()->Close(file);

        bmp_t *bmp = (bmp_t *)buffer.data();
