...
void CScriptObject::WriteToScriptFile(FileHandle_t fp)
{
	...
	switch (type)
	{
		...
		{
			vgui::filesystem()->FPrintf(fp, "\t\t\"%s\"\r\n", prompt);
			vgui::filesystem()->FPrintf(fp, "\t\t{ BOOL }\r\n");
			vgui::filesystem()->FPrintf(fp, "\t\t{ \"%i\" }\r\n", (int)fcurValue ? 1 : 0);
			break;
		}
		...
		{
			vgui::filesystem()->FPrintf(fp, "\t\t\"%s\"\r\n", prompt);
			vgui::filesystem()->FPrintf(fp, "\t\t{ NUMBER %s %s }\r\n", CleanFloat(fMin), CleanFloat(fMax));
		...
		{
			vgui::filesystem()->FPrintf(fp, "\t\t\"%s\"\r\n", prompt);
			vgui::filesystem()->FPrintf(fp, "\t\t{ STRING }\r\n");
		...
		case O_LIST:
		{
			...
			while (pItem)
			{
				...
				vgui::filesystem()->FPrintf(fp, "\t\t\t\"%s\" \"%s\"\r\n", pItem->szItemText, pItem->szValue);
	...
	if (bSetInfo)
		vgui::filesystem()->FPrintf(fp, "\t\tSetInfo\r\n");
...
void CScriptObject::WriteToFile(FileHandle_t fp)
{
	...
	FixupString(cvarname, sizeof(cvarname));
	vgui::filesystem()->FPrintf(fp, "\"%s\"\t\t", cvarname);
	...
	switch (type)
	{
		...
		case O_LIST:
		{
			...
			if (pItem)
			{
				...
				vgui::filesystem()->FPrintf(fp, "\"0.0\"\r\n");
...
CDescription::CDescription(vgui::PanelListPanel*panel)
{
	pObjList = NULL;
...
void CDescription::TransferCurrentValues(const char *pszConfigFile)
{
	...
	while (pObj)
	{
		...
		if (pObj->bSetInfo)
			value = gEngfuncs.LocalPlayerInfo_ValueForKey(pObj->cvarname);
		else
			value = gEngfuncs.pfnGetCvarString(pObj->cvarname);
