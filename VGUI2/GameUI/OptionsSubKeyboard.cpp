...
#include "metahook.h"
#include <GameUI/VControlsListPanel.h>
...
COptionsSubKeyboard::COptionsSubKeyboard(vgui::Panel *parent) : PropertyPage(parent, NULL)
{
	...
	m_pSetBindingButton = new vgui::Button(this, "ChangeKeyButton", "");
	m_pClearBindingButton = new vgui::Button(this, "ClearKeyButton", "");
...
{
	// Create the control
	m_pKeyBindList = new vgui::VControlsListPanel(this, "listpanel_keybindlist");
}
...
void COptionsSubKeyboard::OnCommand( const char *command )
{
	if ( !stricmp( command, "Defaults" )  )
	{
		...
		vgui::QueryBox *box = new vgui::QueryBox("#GameUI_KeyboardSettings", "#GameUI_KeyboardSettingsText");
	...
	{
		m_pKeyBindList->StartCaptureMode(vgui::dc_none);
	}
	else if ( !m_pKeyBindList->IsCapturing() && !stricmp( command, "ClearKey" ) )
	{
		OnKeyCodePressed(vgui::KeyCode::KEY_DELETE);
        m_pKeyBindList->RequestFocus();
	}
	else
	{
	...
	while ( 1 )
	{
		...
		if ( szDescription[ 0 ] != '=' )
		{
			...
			if (!stricmp(szBinding, "blank"))
			{
				...
				m_pKeyBindList->AddColumnToSection(sectionIndex, "Action", szDescription, vgui::SectionedListPanel::COLUMN_BRIGHT, 226);
				m_pKeyBindList->AddColumnToSection(sectionIndex, "Key", "#GameUI_KeyButton", vgui::SectionedListPanel::COLUMN_BRIGHT, 128);
				m_pKeyBindList->AddColumnToSection(sectionIndex, "AltKey", "#GameUI_Alternate", vgui::SectionedListPanel::COLUMN_BRIGHT, 128);
			}
			else
			{
...
{
	gEngfuncs.pfnClientCmd( UTIL_va( "bind \"%s\" \"%s\"\n", key, binding ) );
}
...
{
	gEngfuncs.pfnClientCmd(UTIL_va("unbind \"%s\"\n", key));
}
...
void COptionsSubKeyboard::Finish(int key, int button)
{
	...
	m_pKeyBindList->EndCaptureMode(vgui::dc_arrow);
